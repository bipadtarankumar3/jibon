...
@section('content')
    ...
    <div class="right_part">
        ...
        <form class="common_form" action="{{ route('admin.brrowers.update', $brrowers->user->id) }}" method="post"
                ...
                <section class="pb-4 pt-5">
                    <div class="row gy-3">
                        ...
                        <div class="col-lg-4">
                            <div class="upload_img">
                                <div class="text-center">
                                    ...
                                    <div id="my_camera" style="height: 250;" class="text-center">
                                        ...
                                        @if (isset($brrowers->user->avater))
                                            <img src="{{ URL::to('public/' . $brrowers->user->avater) }}" alt="..."
                                    ...
                                    <div class="form-group mt-2">
                                        ...
                                        @if ($brrowers->user->avater_file)
                                            <img src="{{ 'public/' . $brrowers->user->avater_file }}" height="100"
                                                width="100" alt="">
                                        @endif
                                    </div>
                            ...
                            @if ($brrowers->doucuments && $brrowers->doucuments->count() > 0)
                         
                            @foreach ($brrowers->doucuments as $key => $document)
                           
                                @if ($document && isset($document->image_name))
                                    <div class="attachment-container">
                                        <div class="attachment">
                                            ...
                                            <img src="{{ URL::to('public/upload/' . $document->image_name) }}" alt="Attachment" width="90px">
                                        </div>
                                    </div>
                                @endif
                            @endforeach
                            ...
                            <div class="row gy-3">
                                <div class="col-4">
                                    ...
                                    <input type="text" class="form-control" name="first_name"
                                        value="{{ $brrowers->user->first_name }}">
                                </div>
                                <div class="col-4">
                                    <label>Middle Name (optional) :</label>
                                    <input type="text" class="form-control" name="middle_name"
                                        value="{{ $brrowers->user->middle_name }}">
                                </div>
                                <div class="col-4">
                                    <label>Last Name :</label>
                                    <input type="text" class="form-control" name="last_name"
                                        value="{{ $brrowers->user->last_name }}">
                                </div>
                                <div class="col-12">
                                    <label>Father/Husband Name :</label>
                                    <input type="text" class="form-control" name="father_husband_name"
                                        value="{{ $brrowers->user->father_husband_name }}">
                                </div>
                                <div class="col-4">
                                    <label>Gender :</label>
                                    <select class="form-select" name="gender">
                                        <option value="Male" @if ($brrowers->user->gender == 'Male') selected @endif>Male
                                        </option>
                                        <option value="Female" @if ($brrowers->user->gender == 'Female') selected @endif>Female
                                        </option>
                                    </select>
                                </div>
                                <div class="col-4">
                    ...
                    <div class="row gy-3">
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control" name="city_village"
                                value="{{ $brrowers->address->city }}">
                        </div>
                        <div class="col-6">
                            <label>Market :</label>
                            <select class="form-select" name="market">
                                @foreach ($market as $m)
                                    <option value="{{ $m->id }}" @if ($brrowers->address->market == $m->id) selected @endif>
                                        {{ $m->market_name }}</option>
                                @endforeach
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control" name="post_office"
                                value="{{ $brrowers->address->post_office }}">
                        </div>
                        <div class="col-6">
                            <label>Police Station:</label>
                            <input type="text" class="form-control" name="police_station"
                                value="{{ $brrowers->address->police_station }}">
                        </div>
                        <div class="col-6">
                            <label>Zipcode :</label>
                            <input type="text" class="form-control" name="zipcode"
                                value="{{ $brrowers->address->zip_code }}">
                        </div>
                        <div class="col-6">
                            <label>Country :</label>
                            <input type="text" class="form-control" name="country"
                                value="{{ $brrowers->address->country }}">
                        </div>
                    </div>
                </section>
                <h3>Loan Details</h3>
                    ...
                    <div class="row gy-3">
                            ...
                            <select class="form-select" name="loan_type" onchange="getLoanType(this.value)">
                                ...
                                @foreach ($loan_types as $loan_type)
                                    <option value="{{ $loan_type->id }}"
                                        @if ($brrowers->loan_type_id == $loan_type->id) selected @endif>{{ $loan_type->type_name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12">
                            ...
                            <input type="number" class="form-control" id="principal_amount" name="principal_amount"
                                value="{{ $brrowers->principle_amount }}" oninput="calculateLoan()">
                        </div>
                        <div class="col-6">
                            <label>Loan Terms</label>
                            <input type="text" class="form-control" id="loan_terms" name="loan_terms"
                                value="{{ $brrowers->loan_terms }}" readonly>
                        </div>
                        <div class="col-6">
                            <label>Select</label>
                            <input type="text" id="loan_duration_unit" class="form-control" name="loan_duration_unit"
                                value="{{ $brrowers->days }}" readonly>
                        </div>
                        <div class="col-6">
                            <label>Interest (%)</label>
                            <input type="text" id="interest_rate" class="form-control" name="interest_rate"
                                value="{{ $brrowers->interest }}" readonly>
                        </div>
                        <div class="col-6">
                            <label>Amortization</label>
                            <input type="text" id="amortization" class="form-control" name="amortization"
                                value="{{ $brrowers->amortization }}" readonly>
                        </div>
                        <div class="col-6">
                            <label>Total Amount Loan</label>
                            <input type="text" id="total_loan_amount" class="form-control" name="total_loan_amount"
                                value="{{ $brrowers->total_amount }}" readonly>
                        </div>
                        <div class="col-6">
                            <label>Note</label>
